  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#ffd1e4] px-4 py-8 font-mono text-[#35112f] select-none">
      ...
      <motion.section initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} className="relative z-10 w-full max-w-2xl border-[8px] border-[#2a1028] bg-white p-5 text-center shadow-[10px_10px_0_#f9a8d4] sm:p-7">
        ...
        <motion.div animate={opening ? { rotate: [0, 180, 360, 540], scale: [1, 1.4, 1.9, 14], opacity: [1, 1, 0.95, 0] } : canOpen ? { rotate: [0, 7, -7, 0], scale: [1, 1.08, 1] } : { scale: [1, 1.04, 1] }} transition={{ repeat: opening ? 0 : Infinity, duration: opening ? 1.8 : 2 }} className="relative mx-auto mb-5 mt-5 flex h-28 w-28 items-center justify-center">
          ...
          <img src={canOpen ? starPink : hasAllLetters ? kuromiGif : heartSvg} alt="" className="relative h-24 w-24 object-contain drop-shadow-lg" />
        </motion.div>
        <p className="text-xs font-black uppercase tracking-[0.28em] text-[#be185d]">Portal final</p>
        <h1 className="mt-2 text-3xl font-black uppercase tracking-widest text-[#db2777] sm:text-5xl" style={{ textShadow: '2px 2px 0 #fbcfe8' }}>Abrir o TESOURO!</h1>
        <div className="mx-auto mt-6 max-w-xl border-4 border-[#b6b6b6] bg-[#fff1f7] p-5 shadow-[5px_5px_0_#f9a8d4]">
          <p className="text-sm font-black leading-7 sm:text-base">{canOpen ? 'A palavra-chave esta certinha. O portal reconheceu o brilho.' : hasAllLetters ? 'A palavra ainda esta fora de ordem. Volte ao inventario e arraste as letras.' : `Faltam ${finalPassword.length - clues.length} letras para abrir o tesouro`}</p>
        </div>

        <div className="my-6 grid grid-cols-4 gap-2 sm:grid-cols-7">
