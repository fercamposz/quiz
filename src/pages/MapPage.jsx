...
import { finalPassword, treasures } from '../data/treasures'
...
function TrapModal({ heartsLeft, onClose }) {
  const messages = ['Pegadinha: essa estrela trocou a resposta de lugar.', 'Quase. A Kuromi piscou e confundiu o caminho.', 'Ops. Essa pista era uma isca rosa.']
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-[#100716]/75 p-4 backdrop-blur-md">
      <motion.div initial={{ opacity: 0, scale: 0.82, rotate: -2 }} animate={{ opacity: 1, scale: 1, rotate: 0 }} className="relative w-full max-w-sm border-[6px] border-[#b6b6b6] bg-white p-5 text-center text-[#35112f] shadow-[8px_8px_0_#f9a8d4]">
        {/* Feedback de erro sem o quadrado tremendo no topo. */}
        <motion.img src={kuromiGif} alt="" animate={{ scale: [1, 1.08, 1] }} transition={{ repeat: Infinity, duration: 1.4 }} className="mx-auto mb-4 h-20 w-20 object-contain drop-shadow-xl" />
        ...
        <p className="mb-5 text-sm font-bold text-[#831843]">Vidas restantes: {heartsLeft}. A proxima chance ja foi liberada.</p>
...
export default function MapPage() {
  ...
  const treasureRefs = useRef({})
  const { unlocked, unlockPoint, addClue, completeQuiz, missQuiz, solvedQuiz, missedQuiz, clues, resetGame, hearts, loseHeart } = useGameStore()
  ...
  const finishedStars = solvedQuiz.length + missedQuiz.length
  const progress = Math.round((finishedStars / treasures.length) * 100)
  const currentIndex = Math.min(finishedStars, pathCoordinates.length - 1)
  ...
  useEffect(() => {
    if (!pendingClue) return undefined
    const timer = window.setTimeout(() => {
      setShowClue(pendingClue)
      setPendingClue(null)
    }, 950)
  ...
  function focusTreasure(id) {
    const button = treasureRefs.current[id]
    if (!button) return
    button.focus()
  ...
  function handleMapKeyDown(event) {
    if (activeQuiz || showClue || showCompleted || showGameOver || trap) return
    if (!['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(event.key)) return

    event.preventDefault()
    const unlockedTreasures = treasures.filter((treasure) => unlocked.includes(treasure.id))
    if (!unlockedTreasures.length) return

    const activeId = Number(document.activeElement?.dataset?.treasureId)
    const activeIndex = unlockedTreasures.findIndex((treasure) => treasure.id === activeId)
    const direction = event.key === 'ArrowUp' || event.key === 'ArrowLeft' ? -1 : 1
    const fallbackIndex = direction > 0 ? -1 : unlockedTreasures.length
    const nextIndex = Math.max(0, Math.min(unlockedTreasures.length - 1, (activeIndex === -1 ? fallbackIndex : activeIndex) + direction))

    focusTreasure(unlockedTreasures[nextIndex].id)
  }

  function handleTreasure(id) {
    if (!unlocked.includes(id)) return
    if (solvedQuiz.includes(id) || missedQuiz.includes(id)) {
  ...
  function answerQuiz(answer) {
    ...
    const isLastStar = activeQuiz.id >= treasures.length
    let earnedClue = null
    let nextClueCount = clues.length

    if (answer === activeQuiz.correct) {
      completeQuiz(activeQuiz.id)
      const clue = treasures.find((item) => item.id === activeQuiz.id)
      const nextLetter = finalPassword[clues.length]
      if (clue && nextLetter) {
        earnedClue = { ...clue, letter: nextLetter, clue: `Letra desbloqueada: ${nextLetter}` }
        addClue(earnedClue)
        nextClueCount += 1
      }
    ...
    if (isLastStar && nextClueCount < finalPassword.length) {
      setShowGameOver(true)
    } else if (earnedClue) {
      setPendingClue(earnedClue)
    }
  }
  ...
  return (
    <main onKeyDown={handleMapKeyDown} className="relative min-h-screen overflow-hidden bg-[#ffd1e4] font-mono text-white select-none">
      ...
      <header className="fixed left-0 top-0 z-50 flex w-full items-start justify-between gap-3 p-3 sm:p-4">
        <section className="border-4 border-[#2a1028] bg-white p-3 text-[#35112f] shadow-[5px_5px_0_#f9a8d4]">
          ...
          <p className="mt-2 text-[10px] font-black uppercase tracking-widest sm:text-xs">{finishedStars}/{treasures.length} estrelas</p>
        </section>
      ...
      <div ref={mapRef} onMouseDown={(event) => startDrag(event.pageY)} onMouseMove={(event) => { event.preventDefault(); moveDrag(event.pageY) }} onMouseUp={() => { isDragging.current = false }} onMouseLeave={() => { isDragging.current = false }} onTouchStart={(event) => startDrag(event.touches[0].pageY)} onTouchMove={(event) => moveDrag(event.touches[0].pageY)} onTouchEnd={() => { isDragging.current = false }} className="relative z-10 h-screen w-full cursor-grab overflow-y-scroll overflow-x-hidden active:cursor-grabbing [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <div className="relative h-[1900px] w-full overflow-hidden bg-[#fbcfe8] sm:h-[2200px] lg:h-[2400px]">
          <img src={mapBg} alt="Mapa vertical do Pixelia" draggable="false" className="absolute inset-0 h-full w-full object-cover pointer-events-none" />
          ...
          {treasures.map((treasure, index) => {
            ...
            const isMissed = missedQuiz.includes(treasure.id)
            const isFinalReady = treasure.id === treasures.length && isUnlocked && !isSolved && !isMissed
            return (
              <div key={treasure.id} className={`absolute touch-manipulation ${isFinalReady ? 'z-[70]' : 'z-30'}`} style={{ bottom: position.bottom, left: position.left, transform: 'translate(-50%, 50%)' }}>
                <motion.button ref={(button) => { treasureRefs.current[treasure.id] = button }} data-treasure-id={treasure.id} onClick={() => handleTreasure(treasure.id)} disabled={!isUnlocked} animate={isFinalReady ? { rotate: [0, 12, -12, 0], scale: [1, 1.18, 1] } : isUnlocked && !isSolved && !isMissed ? { y: [-4, 4, -4] } : {}} transition={{ repeat: Infinity, duration: isFinalReady ? 1.25 : 2 }} className="relative block h-20 w-20 outline-none ring-[#2a1028] ring-offset-4 ring-offset-[#fbcfe8] focus-visible:ring-4 sm:h-24 sm:w-24 md:h-32 md:w-32 lg:h-40 lg:w-40 xl:h-44 xl:w-44" aria-label={`Fase ${treasure.id}${isMissed ? ' perdida' : isSolved ? ' completa' : ''}`}>
      ...
      <footer className="fixed bottom-3 left-1/2 z-40 w-[calc(100%-1.5rem)] max-w-2xl -translate-x-1/2 border-4 border-[#2a1028] bg-white p-3 text-center text-xs font-black text-[#35112f] shadow-[5px_5px_0_#f9a8d4] sm:text-sm">Voce tem 3 chances. Acerte para ganhar letras; se errar, perde uma vida e a proxima estrela libera.</footer>
